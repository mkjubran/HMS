...
proces = 16   # Number of processes to create
...
def produceStitch_Framei(f_count,pr_count):
   ii=f_count+pr_count
   if os.path.isfile('tmp/{}.png'.format(format(int(Frame[ii]),'05d'))):
     for FrameStitching in range(1,len(frame_size),skip):
     ...
     cmd = 'mv tmp/{}.png tmp_done/{}.png'.format(format(int(Frame[ii]),'05d'),format(int(Frame[ii]),'05d'))
     output = call(cmd)
...
for filename in os.listdir(input_dir):
    if filename.endswith(".mp4"): 
         ...
         if (( override == 'Y') or (override == 'y')):
            ...
            cmd = 'rm -rf tmp_done' ; output = call(cmd)
            cmd = 'mkdir  tmp_done' ; output = call(cmd)
         ...
         if ( 1 == 1 ):
           ...
           for i in range(0,len(frame_size),skip*proces):
              jobs = []
              for k in range(0,skip*proces,skip):
                out_list = list()
