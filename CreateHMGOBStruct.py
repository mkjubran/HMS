...
print >> fid, 'GOPSize                       : '+str(GOB-1)+'           # GOP Size (number of B slice = GOPSize-1)'
...
ref_pics_RemovedStitching_array=iFNums_array[ref_pics_active_Stitching:GOB]
...
for cnt in range(ref_pics_active_Stitching+1,GOB):
	GOBLine=''
	cnt3=-1
	if cnt < ref_pics_active_Max+1:
		...
		for cnt1 in range(ref_pics_active_Max):
			if cnt1<ref_pics_active_Stitching:
				GOBLine=GOBLine+' '+str(ref_pics_Stitching_array[cnt1]-iFNums_array[cnt])
			else:
				GOBLine=GOBLine+' '+str(cnt3)
