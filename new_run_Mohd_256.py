...
Model = collections.namedtuple("Model", "rate_relative_error, rate_predictor_loss, predict_rate, outputs, predict_real, predict_fake, discrim_loss, discrim_grads_and_vars, gen_loss_GAN, gen_loss_L1, gen_loss_quality, gen_grads_and_vars, train")
...
def main():
    ...
    tf.summary.scalar("generator_loss_quality", model.gen_loss_quality)
    ...
    with sv.managed_session() as sess:
        ...
        if a.mode == "test":
            ...
            # test_summary = tf.Summary()
            test_summary = tf.Summary()
            # test_summary.ParseFromString(fetches["summary"])
            # test_summary.ParseFromString(results["summary"])
            # test_summary_writer.add_summary(test_summary, global_step)
            # sv.summary_writer.add_summary(results["summary"], results["global_step"])
            # sv.summary_writer.add_summary(results["summary"], results["global_step"])
            ...
            # test_summary.value.add(tag='Test Rate Prediction Error (B)', simple_value=avg_rate_pred_err)
            ...
            # sv.summary_writer.add_summary(test_summary, a.max_epochs)
            # sv.summary_writer.add_summary(results["summary"], results["global_step"])
            ...
            # sv.summary_writer.add_summary(test_summary, 100)
                # filesets = save_images(results)
                # for i, f in enumerate(filesets):
                #     print("evaluated image", f["name"])
                # index_path = append_index(filesets)
            ...
            # if should(a.summary_freq):
            #     print("recording summary"), matummary.scalar("generator_loss_L1", model.gen_loss_L1)
            # tf.summary.scalar("generator_loss_quality", model.gen_loss_quality)
            # tf.summary.scalar("rate_predictor_loss", model.rate_predictor_loss)
            # tf.summary.scalar("rate_predictor_distance_from_target (kB)", tf.sqrt(model.rate_predictor_loss))
            #     sv.summary_writer.add_summary(results["summary"], results["global_step"])
