         ...
         try:
           ...
           for i in range(0, len(frame_type), frame_skip):
             ...
             if frame_type[i] != 'I':
                ...
                if ( nrps == 1 ):
                  #-- nrps=1
                  cmd = 'montage tmp/{}.png tmp_e/{}.png tmp_e/{}.png  -tile 3x1 -font DejaVu-Sans -geometry +0+0 PNG24:{}/{}'.format(format(int(Frame[i]),'05d'),format(int(frame_predictors[i][0]),'05d'),format(int(Frame[i]),'05d'),out_dir,fo)
                  output = call(cmd)
                else:
