    ...
    if filename.endswith(".mp4"): 
         ...
         if ( 1 == 1 ):
           ...
           for i in range(1, len(size)):
             #print('{}   {}\n'.format(i,frame_type));
             fo = '{}_{}_SF{}_{}.png'.format(format(int(size[i]), '010d'), format(int(Frame[i]),'05d'), FrameStitching,filename[:-4])
             if frame_type[i] != 'G':
                ...
                if ( nrps == 1):
                  #-- nrps=1
                  cmd = 'montage tmp_e/{}.png tmp_e/{}.png tmp/{}.png  -tile 3x1 -font DejaVu-Sans -geometry +0+0 PNG24:{}/{}'.format(format(int(Frame[i]),'05d'), format(int(FrameStitching),'05d'), format(int(Frame[i]),'05d'),out_dir,fo)
                  output = call(cmd)
                else:
